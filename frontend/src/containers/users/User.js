...
class User extends Component {
	...
	componentDidMount() {
		...
		if (id !== null) {
			this.loadUser(id, (user) => this.setState({ resource: user }));
		} else {
	...
	loadUser(id) {
		this.props.actions.loadUser(id, (resource) =>
			this.setState({ resource: resource, previousUserName: resource.email })
		);
	}
	...
	render() {
		const { resource, validationErrors, previousUserName, passwordConfirm } =
			this.state;
		return (
			<div>
				{resource && (
					<Row className='vertical-middle breadcrumbs'>
						<Col xs={8}>
							<h5>
								<Glyphicon glyph='cog' /> {'Admin > Users > '}
								{resource.id ? (
									<span>
										<b>{previousUserName}</b> - edit
									</span>
								) : (
									<span>New</span>
								)}
